export class LotteryGame {
    ...
    private numberSelectInputs = document.querySelectorAll<HTMLInputElement>('#number-select input');
    private luckyDipButton = document.querySelector<HTMLButtonElement>('#lucky-dip');
    private startGameButton = document.querySelector<HTMLButtonElement>('#start-game');
    private resetButton = document.querySelector<HTMLButtonElement>('#reset');
    private ballContainer = document.querySelector<HTMLDivElement>('#ball-container');
    ...
    private updateSelectedNumbers(): void {
      for (let i = 0; i < this.numberSelectInputs.length; i++) {
        ...
        input.value = this.selectedNumbers[i] ? String(this.selectedNumbers[i]) : '';
      }
    }
    ...
    private checkPrize(): void {
      const numMatches = this.selectedNumbers.filter((number) => this.drawnNumbers.includes(number)).length;
    ...
    private generateRandomNumbers(num: number, specificNumbers: number[] = []): number[] {
